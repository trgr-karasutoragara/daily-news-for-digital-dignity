...
class RSSNewsPrototype:
    def __init__(self, news_max_hours=48, slow_max_days=30, max_workers=12, target_timeout=8):
        ...
        # All English sources for consistent processing - ETH Zurich REMOVED
        self.rss_sources = [
            # Asia Pacific (English editions)
            ("Japan Times", "https://www.japantimes.co.jp/feed", "Japan", "news"),
            ...
            ("Bangkok Post", "https://www.bangkokpost.com/rss/data/news.xml", "Southeast Asia", "slow"),
            ("Straits Times Singapore", "https://www.straitstimes.com/news/world/rss.xml", "Singapore", "news"),
            ("NUS News", "https://news.nus.edu.sg/feed/", "Singapore", "slow"),
            
            # Europe (English editions) - NO ETH ZURICH
            ...
            ("Cambridge University", "https://www.cam.ac.uk/news/feed", "Europe", "slow"),
            ...
            ("MIT News", "https://news.mit.edu/rss/feed", "North America", "slow"),
            ("Stanford News", "https://news.stanford.edu/feed", "North America", "slow"),
            ("Buenos Aires Herald", "https://buenosairesherald.com/rss", "Latin America", "news"),
            ("MercoPress", "https://en.mercopress.com/rss", "Latin America", "news"),
            
            # Africa (English sources)
            ("BBC Africa", "http://feeds.bbci.co.uk/news/world/africa/rss.xml", "Africa", "news"),
            ("AllAfrica", "https://allafrica.com/tools/headlines/rdf/latest/headlines.rdf", "Africa", "slow"),
            ("Mail & Guardian", "https://mg.co.za/rss/", "Africa", "slow"),
            ...
            ("Jerusalem Post", "https://www.jpost.com/rss/rssfeedsheadlines.aspx", "Middle East", "slow"),
            ("Arab News", "https://www.arabnews.com/rss.xml", "Middle East", "news"),
            ("Al-Monitor", "https://www.al-monitor.com/rss", "Middle East", "news"),
            
            # International Organizations (slower update)
            ("UN News", "https://news.un.org/feed/subscribe/en/news/all/rss.xml", "International", "slow"),
            ("International Crisis Group", "https://www.crisisgroup.org/rss", "International", "slow"),
    ...
    def balanced_regional_coverage(self, articles, max_per_region=None):
        ...
        # Updated dynamic limits with enhanced coverage
        region_limits = {
            'International': 30,  # Increased due to 3 sources
            'Europe': 35,         # Increased due to Nordic additions
            'North America': 25,
            'Africa': 25,
            'Middle East': 25,    # Increased due to Al-Monitor addition
            'Latin America': 20,  # Increased due to new sources
            'Environment': 15,
            'Science': 20,        # Increased due to new sources
            'China': 20,
            'Hong Kong': 20,
            'Japan': 20,
            'Singapore': 20,
    ...
    def collect_all_rss_feeds(self):
        ...
        logger.info(f"Starting RSS collection (workers: {self.max_workers})")
...
def main():
    ...
    print("Enhanced regional balance with additional quality sources")
    print()
    ...
    collector = RSSNewsPrototype(
        ...
        slow_max_days=30,     # Specialized sources: 30 days
